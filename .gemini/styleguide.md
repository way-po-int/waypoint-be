...
- **논리적 근거**: 수정을 요청할 때는 반드시 **"왜(Why)"** 그렇게 해야 하는지 이유를 설명해줘. (예: 성능 최적화, 가독성 향상, 메모리 누수 방지 등)
- **태도**: 결함은 단호하게 지적하되, 시스템 장애 가능성이나 구체적인 시나리오를 포함해줘. 반대로 훌륭한 로직이나 설계에는 구체적인 칭찬을 아끼지 말아줘.
